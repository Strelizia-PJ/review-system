...
export default function PomodoroPage() {
  ...
  const phaseColor =
    phase === 'focus' ? 'text-destructive' : phase === 'break' ? 'text-primary' : 'text-muted-foreground'
  ...
  return (
    <div className="mx-auto max-w-md space-y-6 py-4">
      ...
      <div className="flex flex-col items-center">
        <div className="relative h-64 w-64">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className={`mt-2 text-sm font-medium ${phaseColor}`}>{phaseLabel}</span>
            {phase !== 'idle' && (
      ...
      <div className="flex items-center justify-center gap-3">
        ...
        {status === 'running' && (
          <Button size="lg" onClick={pause} className="bg-amber-500 text-white hover:bg-amber-600">
            暂停
          </Button>
        )}
        {status === 'paused' && (
          <>
            <Button size="lg" onClick={resume} className="bg-emerald-500 text-white hover:bg-emerald-600">
              继续
            </Button>
